    ...
    if (store.getters.promise === null) {
      store.commit('promise', new Promise(async (resolve, reject) => {
        ...
        campaigns.forEach(c => {
          ...
          store.commit(campaign + '/compareStart', (new Date(c.dateStart)).getTime() - (new Date()).getTimezoneOffset() * 60 * 1000)
          store.commit(campaign + '/compareEnd', (new Date(c.dateEnd)).getTime() - (new Date()).getTimezoneOffset() * 60 * 1000)
