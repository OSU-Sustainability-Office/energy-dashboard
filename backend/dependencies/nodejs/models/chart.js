...
class Chart {
  ...
  async update(name, point, meterGroup, building, user) {
    ...
    if (user.data.privilege > 3) {
      responseQuery = await DB.query('UPDATE block_groups SET name = ?, point = ?, group_id = ? WHERE id = ?', [
        name,
        point,
        meterGroup,
        this.id,
  ...
  static async create(name, point, meterGroup, building, blockId, user) {
    ...
    let insertRow = await DB.query('INSERT INTO block_groups (name, point, group_id, block_id) VALUES (?, ?, ?, ?)', [
      name,
      point,
      meterGroup,
      blockId,
