...
class Building {
  ...
  async get(expand = true) {
    await DB.connect();
    let buildingRow = await DB.query("SELECT * FROM buildings WHERE id = ?", [
      this.id,
    ]);
    ...
    let meterGroupRows = await DB.query(
      "SELECT id FROM meter_groups where building_id = ?",
      [this.id],
    );
    if (expand) {
  ...
  async update(name, mapId, image, group, meters, user) {
    ...
    if (user.data.privilege > 3) {
      await DB.query(
        "UPDATE buildings SET map_id = ?, image = ?, `group` = ?, name = ? WHERE id = ?",
        [mapId, image, group, name, this.id],
      );
      let first = true;
  ...
  static async create(name, mapId, image, group, meters, user) {
    ...
    await DB.connect();
    let buildingRow = await DB.query(
      "INSERT INTO buildings (map_id, image, `group`, name) VALUES (?, ?, ?, ?)",
      [mapId, image, group, name],
    );
    let meterList = [];
    let first = true;
    for (let meter of meters) {
    ...
    */
    ...
    for (let key of Object.keys(queryJson)) {
      ...
      for (let groupKey of Object.keys(queryJson[key].meterGroups)) {
        let meters = [];
        for (let meterKey of Object.keys(
          queryJson[key].meterGroups[groupKey].meters,
        )) {
