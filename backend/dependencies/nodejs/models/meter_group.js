...
class MeterGroup {
  ...
  async get(expand = false) {
    await DB.connect();
    let meterGroupRow = await DB.query(
      "SELECT * FROM meter_groups WHERE id = ?",
      [this.id],
    );
    ...
    let metersRow = await DB.query(
      "SELECT meter_id FROM meter_group_relation WHERE group_id = ?",
      [this.id],
    );
    if (expand) {
  ...
  async update(name, meters, def, user) {
    ...
    await DB.connect();
    await Promise.all([
      DB.query("UPDATE meter_groups SET name = ?, `default` = ? WHERE id = ?", [
        name,
        def ? 1 : 0,
        this.id,
      ]),
      DB.query("DELETE FROM meter_group_relation WHERE group_id = ?", [
        this.id,
      ]),
    ]);
    this.name = name;
    ...
    for (let meter of meters) {
      ...
      DB.query(
        "INSERT INTO meter_group_relation (meter_id, group_id, operation) VALUES (?, ?, ?)",
        [meter.id, this.id, meter.operation],
      );
    }
    // this.meters = await Promise.all(this.meters)
    return this;
  ...
  static async deleteBuildingGroups(building, keepList, user) {
    if (user.data.privilege > 3) {
      ...
      try {
        ...
        await DB.query(
          "DELETE FROM meter_groups WHERE NOT (id IN " +
            keepString +
            ") AND building_id_2 = ? AND building_id IS NULL",
          [building],
        );
      } catch (e) {
        console.log(e);
  ...
  static async create(name, meters, def, building, user) {
    ...
    let insertRow = await DB.query(
      "INSERT INTO meter_groups (name, building_id_2, `default`) VALUES (?, ?, ?)",
      [name, building, def ? 1 : 0],
    );
    ...
    for (let meter of meters) {
      meterPromises.push(
        DB.query(
          "INSERT INTO meter_group_relation (meter_id, group_id, operation) VALUES (?, ?, ?)",
          [meter.id, insertRow["insertId"], meter.operation],
        ),
      );
    }
