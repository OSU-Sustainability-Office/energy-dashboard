...
class MeterGroup {
  ...
  async update (name, meters, def, user) {
    ...
    for (let meter of meters) {
      ...
      DB.query('INSERT INTO meter_group_relation (meter_id, group_id, operation) VALUES (?, ?, ?)', [meter.id, this.id, meter.operation])
    }
    // this.meters = await Promise.all(this.meters)
    return this
  }
  ...
  static async deleteBuildingGroups (building, keepList, user) {
    if (user.data.privilege > 3) {
      ...
      try {
        await DB.query('DELETE meter_group_relation FROM meter_group_relation INNER JOIN meter_groups ON meter_groups.id = meter_group_relation.group_id WHERE NOT (meter_groups.id IN ' + keepString + ') AND meter_groups.building_id_2 = ? AND meter_groups.building_id IS NULL', [building])
        await DB.query('DELETE FROM meter_groups WHERE NOT (id IN ' + keepString + ') AND building_id_2 = ? AND building_id IS NULL', [building])
      } catch (e) {
        console.log(e)
      }
    } else {
  ...
  static async create (name, meters, def, building, user) {
    ...
    await DB.connect()
    let insertRow = await DB.query('INSERT INTO meter_groups (name, building_id_2, `default`) VALUES (?, ?, ?)', [name, building, (def) ? 1 : 0])
    console.log(insertRow)
    let meterPromises = []
    for (let meter of meters) {
      meterPromises.push(
        DB.query('INSERT INTO meter_group_relation (meter_id, group_id, operation) VALUES (?, ?, ?)', [meter.id, insertRow['insertId'], meter.operation])
      )
    }
