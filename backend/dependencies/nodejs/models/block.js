...
class Block {
  ...
  async get(expand = true) {
    await DB.connect();
    let blockRow = await DB.query("SELECT * FROM blocks WHERE id = ?", [
      this.id,
    ]);
    ...
    let chartRows = await DB.query(
      "SELECT id FROM block_groups WHERE block_id = ?",
      [this.id],
    );
    if (expand) {
  ...
  async delete(user) {
    ...
    let responseQuery;
    if (user.data.privilege > 3) {
      responseQuery = await DB.query("DELETE FROM blocks WHERE id = ?", [
        this.id,
      ]);
    } else {
      responseQuery = await DB.query(
  ...
  static async create(
    ...
    await DB.connect();
    let userCheck = await DB.query("SELECT user FROM stories WHERE id = ?", [
      storyId,
    ]);
