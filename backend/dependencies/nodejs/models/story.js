...
class Story {
  ...
  async update(name, media, user) {
    ...
    if (user.data.privilege > 3) {
      queryResponse = await DB.query('UPDATE stories SET name = ?, media = ? WHERE id = ? AND user = ?', [
        name,
        media,
        this.id,
        user.onid,
  ...
  static async create(name, media, user) {
    await DB.connect();
    let insertRow = await DB.query('INSERT INTO stories (name, media, user) VALUES (?, ?, ?)', [
      name,
      media,
      user.onid,
    ]);
  ...
  static async storiesForUser(user) {
    await DB.connect();
    let storyRows = await DB.query('SELECT * FROM stories WHERE user = ?', [user]);
    let stories = [];
