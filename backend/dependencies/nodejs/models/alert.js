...
class Alert {
  ...
  async get() {
    await DB.connect();
    let alertRow = await DB.query("SELECT * FROM alerts WHERE id = ?", [
      this.id,
    ]);
  ...
  async update(lowThreshold, highThreshold, point, meterId, user) {
    ...
    if (user.privilege > 3) {
      responseQuery = await DB.query(
        "UPDATE alerts SET low = ?, high = ?, point = ?, meter_id = ? WHERE id = ?",
        [lowThreshold, highThreshold, point, meterId, this.id],
      );
    } else {
      responseQuery = await DB.query(
  ...
  async delete(user) {
    ...
    if (user.privilege > 3) {
      ...
      responseQuery = await DB.query(
        "DELETE alerts WHERE id = ? AND user = ?",
        [this.id, user.onid],
      );
    }
    if (responseQuery["affectedRows"] === 0) {
  ...
  static async create(lowThreshold, highThreshold, point, meterId, user) {
    ...
    let insertRow = await DB.query(
      "INSERT INTO alerts (user, low, high, point, meter_id) VALUES (?, ?, ?, ?, ?)",
      [user.onid, lowThreshold, highThreshold, point, meterId],
    );
  ...
  static async forMeter(meter) {
    await DB.connect();
    let alertRows = await DB.query("SELECT * FROM alerts WHERE meter_id = ?", [
      meter.id,
    ]);
    let returnAlerts = [];
