...
class Meter {
  ...
  async get() {
    ...
    if (this.address && this.address !== "") {
      row = await DB.query("SELECT * FROM meters WHERE address = ?", [
        this.address,
      ]);
    } else {
  ...
  async download(point, startTime, endTime, meterClass) {
    ...
    if (Object.values(meterClasses[meterClass]).includes(point)) {
      ...
      if (String(meterClass).startsWith("999")) {
        ...
        let [{ name: meter_table_name }] = await DB.query(
          "SELECT `name` FROM meters WHERE id = ?",
          [this.id],
        );
  ...
  async upload(data) {
    ...
    const timeseconds =
      new Date(time).getTime() / 1000 - new Date().getTimezoneOffset() * 60;
    try {
      await DB.query(
  ...
  async update(name, classInt, negate, user) {
    ...
    await DB.connect();
    await DB.query(
      "UPDATE meters SET name = ?, class = ?, negate = ? WHERE id = ?",
      [name, classInt, negate, this.id],
    );
    this.name = name;
    this.classInt = classInt;
    this.negate = negate;
  ...
  static async create(name, address, classInt, negate = 0) {
    await DB.connect();
    let returnRow = await DB.query(
      "INSERT INTO meters (name, address, class, negate) values (?, ?, ?, ?)",
      [name, address, classInt, negate],
    );
