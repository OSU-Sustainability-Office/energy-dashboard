...
exports.post = async (event, context) => {
  ...
  try {
    response.body = JSON.stringify((await MeterGroup.create(event.body.name, event.body.meters, user)).data)
  } catch (error) {
...
exports.put = async (event, context) => {
  ...
  let user = new User(event, response)
  try {
    await MeterGroup(event.body.id).update(event.body.name, event.body.meters, user)
  } catch (error) {
    response.body = error.message
    response.status = 400
  }
