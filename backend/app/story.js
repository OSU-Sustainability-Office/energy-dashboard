...
exports.get = async (event, context) => {
  ...
  response.body = JSON.stringify(
    (await new Story(event.queryStringParameters["id"]).get()).data,
  );
  return response;
};
...
exports.post = async (event, context) => {
  ...
  try {
    response.body = JSON.stringify(
      (await Story.create(event.body.name, event.body.media, user)).data,
    );
  } catch (err) {
...
exports.put = async (event, context) => {
  ...
  await user.resolved;
  try {
    await new Story(event.body.id).update(
      event.body.name,
      event.body.media,
      user,
    );
