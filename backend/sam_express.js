...
(async () => {
  ...
  const awsKeys = (() => {
    const fileContents = FileSystem.readFileSync(
      require("os").homedir() + "/.aws/credentials",
    ).toString();
  ...
  for (const resourceKey of Object.keys(template.Resources)) {
    ...
    if (resource.Type === "AWS::Serverless::Function") {
      const routeProperties = Object.values(resource.Properties.Events)[0]
        .Properties;
      app[routeProperties.Method](
        ...
        async (req, res) => {
          ...
          const fullModPath =
            "./" + resource.Properties.CodeUri + codePath[0] + ".js";
