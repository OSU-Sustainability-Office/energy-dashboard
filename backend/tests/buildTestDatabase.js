...
DB.beginTransaction((err) => {
  ...
  */
  DB.query(gigantic_query_string, (err) => {
    ...
    DB.query(
      formatInserts("tests/assertedData/buildings_insert.sql", "buildings"),
      (err) => {
        if (err)
          return DB.rollback(() => {
            throw err;
        ...
        DB.query(
          formatInserts(
            "tests/assertedData/meter_groups_insert.sql",
            "meter_groups",
          ),
          (err) => {
            if (err)
            ...
            DB.query(
              formatInserts(
                "tests/assertedData/meter_group_relation_insert.sql",
                "meter_group_relation",
              ),
              (err) => {
                if (err)
                ...
                DB.query(
                  formatInserts(
                    "tests/assertedData/meters_insert.sql",
                    "meters",
                  ),
                  (err) => {
                    if (err)
                    ...
                    DB.query(
                      formatInserts(
                        "tests/assertedData/campaigns_insert.sql",
                        "campaigns",
                      ),
                      (err) => {
                        if (err)
                        ...
                        DB.query(
                          formatInserts(
                            "tests/assertedData/campaign_groups_insert.sql",
                            "campaign_groups",
                          ),
                          (err) => {
                            if (err)
                            ...
                            console.log(
                              "added data to the campaign_groups table!",
                            );

                            DB.query(
                              formatInserts(
                                "tests/assertedData/data_insert.sql",
                                "data",
                              ),
                              (err) => {
                                if (err)
