...
app.post('/upload', xmlparser({
  ...
  console.log(req.body.das);
    if (req.body.das && req.body.das.mode == 'LOGFILEUPLOAD') {
        ...
        req.body.das.devices.device.forEach(function (device) {
          ...
          connection.query('SELECT id FROM meters WHERE address = "' + meter_id + '"', function(err, res) {
            ...
            if (res.length == 0) {
              ...
              connection.query('INSERT INTO meters (Name, address, building_id, operation) VALUES ("' + device.name + '","' + meter_id + '", NULL, 1)');
            }

            // Iterate over each record being uploaded
            device.records.record.forEach(function(record) {
              // Now, add temp data.
